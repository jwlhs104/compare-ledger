class BaseReconciliationLogic {
  ...
  getPriceFromGoogleSheet(roomTypeText, peopleAmount, name, date, orderNumber) {
    try {
      // Use cached data
    ...
    try {
      ...
      if (typeof rawPrice === "object" && rawPrice.price) {
        ...
        return {
          price: null,
          data: null,
          error: `${roomTypeText}-${roomPeopleCount}入住-無報價`,
        };
      } else {
        return {
          price: null,
          data: null,
          error: `${roomTypeText}-${roomPeopleCount}入住-查無房型報價`,
        };
      }
    } catch (e) {
      return {
  ...
  formatRoomType(isSpecialPricing, specialPriceData, room, totalNights, index) {
    if (isSpecialPricing && specialPriceData) {
      ...
      if (index === 0 && room.date === room.firstDate) {
        return `${specialPriceData.orderNumber}${
      ...
      stayRooms.forEach((room) => {
        ...
        room.people.forEach((person, index) => {
          ...
          if (isSpecialPricing) {
            ...
            const regularResult = this.getRegularPricingData(
              roomTypeText,
              room.people.length,
              person,
              room.date
          ...
          if (price) {
            const roomType = this.formatRoomType(
              isSpecialPricing,
              specialPricingResult?.data,
              room,
              totalNights,
              index
            );
            this.processRoomType(
              ageGroupData,
              roomType,
              price,
              room,
...
class BaseSnowTicketReconcileLogic {
