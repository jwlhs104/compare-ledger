class NaebaInvoiceParser extends BaseInvoiceParser {
  parse(sheet) {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      stayContext = this.resetContextIfNewGuest(stayContext, guestInfo.continuation);
      
      for (let day = 0; day < 6; day++) {
        const dayInfo = this.extractDayInfo(row, day);
        
        if (notNull(dayInfo.roomType)) {
          const scenario = this.detectScenario(dayInfo.roomType, stayContext.previousRoomType);
          stayContext = this.handleScenario(scenario, dayInfo, stayContext, rooms, roomIndexs, guestInfo);
        } else {
